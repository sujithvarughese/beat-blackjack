...
const GameProvider = ({ children }) => {
    ...
    const dealHands = () => {
        ...
        // handle blackjacks, show hidden dealer card, and display results
            ...
            const netProfit = Number(currentBet) * Number(state.blackjackPayout)
            status = {
                ...status,
                winner: 1,
                resultsShown: true,
    ...
    const determineWinner = () => {
        const netBet = state.doubledHand ?  Number(state.bet) * 2 : Number(state.bet)
        let winner = 0
        ...
        if (dealerScore > 21 || playerScore > dealerScore) {
            winner = 1
            status = { ...status, playerBankroll: Number(state.playerBankroll) + netBet + netBet}
        } else if (dealerScore > playerScore) {
        } else {
            if (state.doubledHand) {
                status = { ...status, playerBankroll: Number(state.playerBankroll) + state.bet * 2}
            } else {
                status = { ...status, playerBankroll: Number(state.playerBankroll) + state.bet }
            }
        }
        dispatch({
